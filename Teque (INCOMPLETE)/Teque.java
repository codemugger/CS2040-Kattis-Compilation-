...
public class Teque{
  ...
  // add items to the list
    ...
    public static void main(String[] args) throws IOException{
        ...
        TailedLinkedList specialQueue = new TailedLinkedList();
        ...
        //HashMap<Integer,ListNode> outputMapping = new HashMap<Integer,ListNode>(); 

        int getIndex = 0;

        String[] userInput;

        //int[] outputArr = new int[1000000];
        ...
        for (int i = 0; i < numberOfOperations; i++){
            ...
            if (function.equals(pushBack)) {specialQueue.addBack(number);}

            else if (function.equals(pushFront)) {specialQueue.addFront(number);}

            else if (function.equals(pushMiddle)) {specialQueue.addAtIndex((specialQueue.size() + 1)/2,number);}

            else {
                //outputArr[getIndex] = specialQueue.getItemAtIndex((int) number);
                //getIndex++;
                sbf.append(specialQueue.getItemAtIndex((int) number)).append("\n");
            } 

        }

        //for (int i = 0 ; i < getIndex; i++) {writer.println(outputArr[i]);}
        writer.print(sbf);
        writer.flush();
...
class TailedLinkedList {
  // attributes
  public ListNode head; // track front with head 
  public ListNode tail; // track back with tail 
  public int num_nodes; // track the number of nodes in the TLL 
  ...
  public int getItemAtIndex(int index) {
    ...
    if (index < 0 || index > size()-1) { // check index validity, must be within bound 
      System.out.println("invalid index");
      System.exit(1);
    }
    if (index == size()-1)
      item = tail.getItem(); // since is a tailed list, might as well make use of tail if it's last index! 
    else {
  ...
  public void  addAtIndex(int index, int item) {
    ListNode cur;
    ListNode newNode = new ListNode(item); // create new node with added item, initially pointing to null 

    if (index >= 0 && index <= size()) { // check index within bound 
  ...
  public int remove(ListNode cur) {
    ...
    num_nodes--; // rmb bookkeeping the values is very important 
